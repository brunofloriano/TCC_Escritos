...
Para tal, deve-se igualar a posição desejada em regime permanente, $\theta_i^{\ast}[k]$, à posição requerida pela marcha a cada instante k. Utilizaremos uma das marchas desenvolvidas em \cite{tcc:porphirio:2017} para esta implementação.
...
Estes dois fatores (lentidão de resposta e movimento cadenciado) pode ser explicado pelo tempo de amostragem adotado neste trabalho. Quando desenvolvido em \cite{tcc:porphirio:2017}, a marcha utilizada não foi implementada com um período específico de amostragem como no sistema desenvolvido.

Dessa forma, tal marcha, implementada como foi originalmente concebida, não está adequada aos parâmetros do sistema proposto.
