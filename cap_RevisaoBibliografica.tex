...
\begin{figure}[h]
	...
	\caption{Polígonos de Suporte de um robô quadrúpede (Fonte: \cite{article:mcghee:1968})}
	\label{fig:poligonos}
\end{figure}
...
\begin{figure} [b]
	...
	\caption{Robô prestes a tombar (Fonte: \cite{book})}
	\label{fig:dinamico1}
\end{figure}
...
\begin{figure} [t]
	...
	\caption[Robô de uma perna]{Robô de uma perna (Fonte: \footnotemark)}
	\label{fig:umaperna}
\end{figure}
...
\begin{figure} [b]
	...
	\caption{Perna Virtual (Fonte: \cite{article:raibert:1986})}
...
\begin{align}
x_{f,d} &= \frac{\dot{x}T_S}{2} + k_{\dot{x}}(\dot{x}-\dot{x_d}), \text{\space e} \\
...
\begin{align}
u_x &= -k_{p,x}(\phi_P-\phi_{P,d}) - k_{v,x}(\dot{\phi_P}) - k_{f,x}(f_x), \text{\space e}\\
...
\begin{align}
M\ddot{r} + K(r-r_0) - Mr\dot{\theta^2} &= -Mg\cos\theta, \text{\space e} \\
...
\begin{align} 
\theta_s &= \arcsin(\frac{v_xT_s}{2l_0}) + k_p(v_x-v_{xt}) + k_i\sum_{step}(v_x-v_{xt}) \label{eq:slip2}, \text{\space e}\\
...
\begin{figure} [t]
	...
	\caption{Modelo para controle de arfagem e rolagem (Fonte: \cite{article:li:2014})}
	\label{fig:slip3}
\end{figure}
...
\begin{align}
\theta_F &= \theta_{FS} - \theta_{corpo}, \text{\space e}\\
...
\begin{figure} [h]
	...
	\caption{Modelo das relações de ângulo entre perna e corpo (Fonte: \cite{article:meng:2015})}
	\label{fig:balanco2}
\end{figure}
...
\begin{figure} [b]
	...
	\caption{Estrutura do sistema de controle (Fonte: \cite{article:meng:2015})}
	\label{fig:balanco3}
\end{figure}
...
\begin{figure} [t]
	...
	\caption{Diagrama de Blocos do Sistema de Controle (Fonte: \cite{article:won:2009})}
	\label{fig:lar3}
\end{figure}
...
\begin{align}
\tau_{\phi} &= k_{p,x}\Delta\phi + k_{d,x}\Delta\dot{\phi}, \text{\space e}\\
...
\begin{align}
G_v &= \frac{\Delta \dot{c}_x}{\Delta \dot{q}_2},  \\ G_{\omega} &= \frac{\Delta \dot{\phi}}{\Delta \dot{q}_2}, \text{\space e}
\end{align}
...
\begin{align}
f_{roll,i,p} &= k_{roll}f_i(\phi_{roll})\label{empirico1}, \text{\space e}  \\
