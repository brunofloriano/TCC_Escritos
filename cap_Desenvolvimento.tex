...
Uma vez detectada resposta do teclado, a função \emph{main} cria um timer com tempo de amostragem $T_{am}$ que é configurado para chamar a função de controle sempre $T_{am}$ segundos forem decorridos. Com isso, a \textit{thread} de controle é criada e a função \emph{main}, a partir de então, permanece em um loop apenas para verificar se a condição de parada foi satisfeita (neste caso a condição de parada é um novo toque em qualquer tecla) e atualizar o gDataLogger.
